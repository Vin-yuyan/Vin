...
public class OutputWord {
	public static void main(String args[]){
		...
		frequency.setFileName("d:/hello.txt");
		frequency.wordFrequency();
		allWord = frequency.getAllWord();
		noSameWord = frequency.getNoSameWord();
		...
		for(int m=0;m<noSameWord.size();m++){
			double frequence = (1.0*count[m])/allWord.size();
			System.out.printf("%s:%-7.3f",noSameWord.elementAt(m),frequence);
		}
	}
